...
async def cb_handler(client, query: CallbackQuery):
    ...
    if data == "check_join":
        ...
        await query.message.edit(f"Select Payment Method for {day} Day Plan:",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💳 PAY WITH UPI", callback_data=f"pay_upi_{day}")], [InlineKeyboardButton("💰 PAY WITH BINANCE", callback_data=f"pay_bin_{day}")]]))
...
async def photo_handler(client, message):
    ...
    await client.send_message(ADMIN_ID, f"New Payment Request\nUser ID: `{message.from_user.id}`", 
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"Approve {d} Day", callback_data=f"approve_{message.from_user.id}_{d}")] for d in [1, 7, 15, 30]] + [[InlineKeyboardButton("Reject", callback_data="reject")]]))
